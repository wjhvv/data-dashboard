...
import { splitByGroup, computeGroupedCategoricalData, GROUP_COLORS } from '../../utils/groupUtils';
...
export function GroupedCategoricalChart({ rows, colIdx, groupColIdx, groupColName }: Props) {
  const { data, groups } = useMemo(() => {
    const grouped = splitByGroup(rows, groupColIdx);
    return {
      groups: Object.keys(grouped),
      data: computeGroupedCategoricalData(grouped, colIdx),
    };
  }, [rows, colIdx, groupColIdx]);

  if (data.length === 0) {
    return <div className="flex items-center justify-center h-32 text-gray-400 text-xs">No data</div>;
  }
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 48 }}>
          <XAxis dataKey="label" tick={{ fontSize: 10 }} angle={-45} textAnchor="end" tickMargin={6} interval={0} />
